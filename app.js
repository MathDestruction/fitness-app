...
const fmtInt = (n, u = "") =>
  Number.isFinite(n) ? `${Math.round(n)}${u}` : "-";
const dateFmt = (d) =>
...
function applyThemeColor(c) {
  ...
  if (c === "blue") {
    r.style.setProperty("--accent", "#4CB7FF");
    r.style.setProperty("--accent-hover", "#2FA6F4");
    r.style.setProperty("--accent-dim", "rgba(76,183,255,0.14)");
  } else {
    r.style.setProperty("--accent", "#86C8A3");
    r.style.setProperty("--accent-hover", "#72B691");
    r.style.setProperty("--accent-dim", "rgba(134,200,163,0.12)");
  }
}
...
function inDays(ds, d) {
  return (new Date(today) - new Date(ds)) / dayMs <= d - 1;
}
...
$("#dev-login-btn").addEventListener("click", async () => {
  const email = window.DEV_EMAIL,
    password = window.DEV_PASSWORD,
    errEl = $("#auth-error"),
    btn = $("#dev-login-btn");
  if (!email || !password) {
    errEl.textContent = "Dev credentials not set in index.html";
    return;
  }
  btn.innerHTML = '<span class="spinner"></span> Bypassing…';
  btn.disabled = true;
  errEl.textContent = "";
  ...
  btn.innerHTML = "Dev Login (Bypass)";
  btn.disabled = false;
  if (error) {
    errEl.style.color = "var(--danger)";
...
function rowsToSession(sRow, eRows, ssRows) {
  const entries = eRows.map((e) => {
    ...
    if (e.type === "strength") {
      ...
      return {
        ...
        sets,
        muscle:
          EXERCISE_DB.strength.find((x) => x.name === e.exercise_name)
            ?.muscle || "Other",
      };
    }
    return {
  ...
  return {
    id: sRow.id,
    sessionDate: sRow.session_date,
    weightKg: sRow.weight_kg != null ? Number(sRow.weight_kg) : null,
    notes: sRow.notes || "",
    entries,
  };
}
...
async function loadSessions() {
  ...
  state.sessions = sessions.map((s) =>
    rowsToSession(
      s,
      entries.filter((e) => e.session_id === s.id),
      ss,
    ),
  );
}
...
async function ensureSession() {
  ...
  if (error || !session) {
    const { error: re } = await _supabase.auth.refreshSession();
    if (re) throw new Error("Session expired. Please sign in again.");
  }
}
...
async function insertEntries(sid, entries) {
  for (let i = 0; i < entries.length; i++) {
    ...
    if (e.type === "cardio") {
      row.duration_seconds = e.durationMin
        ? Math.round(e.durationMin * 60)
        : null;
      row.distance_km = e.distanceKm || null;
      row.speed_kmh = e.speedKmh || null;
    } else if (e.type === "hold") {
      row.hold_sets = e.sets || 1;
      row.hold_seconds_per_set = e.seconds || 0;
    }
    const {
      data: [ne],
      error: eErr,
    } = await _supabase.from("entries").insert(row).select();
    if (eErr) throw eErr;
    if (e.type === "strength" && e.sets?.length) {
      const sr = e.sets.map((s, si) => ({
        entry_id: ne.id,
        set_index: si,
...
function createChart(k, ctx, c) {
  destroyChart(k);
  state.charts[k] = new Chart(ctx, c);
}
...
const ICONS = {
  ...
  cycle:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="6" cy="17" r="3"/><circle cx="18" cy="17" r="3"/><path d="M6 17l3-7h4l3 7"/></svg>',
  ...
  dumbbell:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6.5 6.5h11M6 12h12M6.5 17.5h11M4 6.5V17.5M20 6.5V17.5M2 9v6M22 9v6"/></svg>',
  squat:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="4" r="2"/><path d="M8 22v-4l-2-4 3-4h6l3 4-2 4v4"/></svg>',
  timer:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="13" r="9"/><polyline points="12 7 12 13 15 15"/><path d="M10 2h4"/></svg>',
...
function renderDashboard() {
  ...
  $("#last-session-text").textContent = last
    ? `Last Session - ${dateFmtShort(last.sessionDate)}`
...
function renderGoals(sessions) {
  ...
  const now = new Date(),
    dow = (now.getDay() + 6) % 7;
  const monDate = new Date(now.getTime() - dow * dayMs)
    .toISOString()
    .slice(0, 10);
  const weekDates = new Set(
    sessions
      .filter((s) => s.sessionDate >= monDate && s.weightKg != null)
      .map((s) => s.sessionDate),
  );
  ...
  const fives = runs
    .filter((r) => Math.abs(r.dist - 5) < 0.5 && r.dur > 0)
    .sort((a, b) => a.dur - b.dur);
  const best5 = fives[0],
    target = 30;
  const timeEl = $("#goal-5k-time"),
    arcEl = $("#goal-5k-arc");
  if (best5) {
...
function renderStats(filtered) {
  ...
  const runs = filtered.flatMap((s) =>
    s.entries
      .filter((e) => e.type === "cardio")
      .map((e) => ({ dist: calcDistance(e), dur: e.durationMin || 0 })),
  );
  ...
  $("#stat-time").textContent =
    totalMin > 0 ? `${hrs}:${String(mins).padStart(2, "0")}` : "-";
...
function renderPlanks() {
  const textEl = $("#plank-text");
  const barEl = $("#plank-bar");
  const streakEl = $("#plank-streak-text");
  if (!textEl || !barEl) return;
  const planksToday = state.sessions
    .filter((s) => s.sessionDate === today)
    .flatMap((s) =>
      s.entries.filter(
        (e) => e.type === "hold" && e.exercise.toLowerCase() === "plank",
      ),
    )
    .reduce((acc, e) => acc + (e.sets || 1), 0);
  const pct = Math.min(100, (planksToday / 10) * 100);
  textEl.textContent = `${planksToday}/10`;
  barEl.style.width = `${pct}%`;

  if (streakEl) {
    let streak = 0;
    const todayStr = today;
    const dates = [...new Set(state.sessions.map((s) => s.sessionDate))]
      .sort()
      .reverse();

    // Check if achieved today
    const countToday = state.sessions
      ...
      .reduce((a, e) => a + (e.sets || 1), 0);
    const hasToday = countToday >= 10;

    // Check yesterday
    const yesterdayDate = new Date(new Date().getTime() - dayMs);
    const yesterdayStr = yesterdayDate.toISOString().slice(0, 10);
    const countYesterday = state.sessions
      .filter((s) => s.sessionDate === yesterdayStr)
      .flatMap((s) =>
        s.entries.filter(
          (e) => e.type === "hold" && e.exercise.toLowerCase() === "plank",
        ),
      ...
      .reduce((a, e) => a + (e.sets || 1), 0);
    const hasYesterday = countYesterday >= 10;
    ...
    // Traverse backward checking dates
    ...
    while (true) {
      const dStr = new Date(curTime).toISOString().slice(0, 10);
      const plankCount = state.sessions
        .filter((s) => s.sessionDate === dStr)
        .flatMap((s) =>
          s.entries.filter(
            (e) => e.type === "hold" && e.exercise.toLowerCase() === "plank",
          ),
...
function getExerciseCategory(entry) {
  ...
  const ex = EXERCISE_DB.strength.find(
    (s) => s.name.toLowerCase() === (entry.exercise || "").toLowerCase(),
  );
...
function renderMuscleBalance(filtered) {
  ...
  if (total === 0) {
    ...
    legendEl.innerHTML =
      '<span class="donut-legend-item" style="color:var(--muted)">No data yet</span>';
...
function renderHeatmap(allSessions) {
  ...
  const actualDays = Math.min(
    totalDays,
    Math.ceil((now - startDate) / dayMs) + 1,
  );
...
function renderRecovery(filtered) {
  ...
  if (sessions.length < 2) {
    ...
    statsEl.innerHTML =
      '<span class="recovery-stat-item"><span class="recovery-stat-label">Need more data</span><span class="recovery-stat-value">Log 2+ sessions</span></span>';
    return;
  }

  const labels = sessions.map((s) => dateFmtShort(s.sessionDate));
  const volumes = sessions.map((s) => strengthVolume(s));
  const cardioMins = sessions.map((s) =>
    s.entries
      .filter((e) => e.type === "cardio")
      .reduce((a, e) => a + (e.durationMin || 0), 0),
  );
  ...
  let fatigueClass = "recovery-good",
    fatigueLabel = "Fresh";
  if (avgRest < 1 && avgVol > 3000) {
    fatigueClass = "recovery-high";
    fatigueLabel = "High Load";
  } else if (avgRest < 2 && avgVol > 2000) {
    fatigueClass = "recovery-moderate";
    ...
    options: {
      ...
      scales: {
        ...
        y1: {
          ...
          ticks: { font: { size: 9 }, callback: (v) => v + "d" },
        },
      },
    },
  });
...
function renderRunningAnalytics(sessions) {
  ...
  sessions.forEach((s) => {
    s.entries
      ...
      .forEach((e) => {
        ...
        if (isRun || isWalk) {
          if (!cardioByDate.has(s.sessionDate))
            cardioByDate.set(s.sessionDate, {
              runDist: 0,
              runDur: 0,
              walkDist: 0,
              walkDur: 0,
          ...
          if (isRun) {
            d.runDist += calcDistance(e);
            d.runDur += e.durationMin || 0;
          }
          if (isWalk) {
            d.walkDist += calcDistance(e);
            d.walkDur += e.durationMin || 0;
          }
        }
      });
  });
  ...
  if (statEl)
    statEl.textContent =
      latest && latest.runSpeed > 0 ? `${latest.runSpeed.toFixed(1)} km/h` : "";

  drawRunChart(aggregated, state.activeRunChart);
}

function drawRunChart(data, mode) {
  const canvas = $("#run-chart"),
  ...
  runGrad.addColorStop(0, "rgba(134,200,163,0.35)");
  runGrad.addColorStop(1, "rgba(134,200,163,0.02)");

  const walkGrad = ctx.createLinearGradient(0, 0, 0, h);
  walkGrad.addColorStop(0, "rgba(96,165,250,0.35)");
  walkGrad.addColorStop(1, "rgba(96,165,250,0.02)");
  ...
  const commonOpts = {
    ...
    plugins: {
      legend: {
        display: true,
        position: "top",
        labels: { boxWidth: 10, padding: 12, color: "#c1cdd6" },
      },
    },
  };
    ...
    config = {
      ...
      data: {
        ...
        datasets: [
          {
            label: "Run",
            data: data.map((d) => d.runDist || null),
            borderColor: "#86C8A3",
          ...
          {
            label: "Walk",
            data: data.map((d) => d.walkDist || null),
            borderColor: "#60A5FA",
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              title: (i) =>
                data[i[0].dataIndex] ? dateFmt(data[i[0].dataIndex].date) : "",
              label: (i) => `${i.dataset.label}: ${i.parsed.y.toFixed(2)} km`,
            },
          },
        },
        scales: {
    ...
    config = {
      ...
      data: {
        ...
        datasets: [
          {
            label: "Run Speed",
            data: data.map((d) => d.runSpeed || null),
            borderColor: "#86C8A3",
            backgroundColor: runGrad,
            fill: true,
            tension: 0.4,
          ...
          {
            label: "Walk Speed",
            data: data.map((d) => d.walkSpeed || null),
            borderColor: "#60A5FA",
            backgroundColor: walkGrad,
            fill: true,
            tension: 0.4,
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              title: (i) =>
                data[i[0].dataIndex] ? dateFmt(data[i[0].dataIndex].date) : "",
              label: (i) => `${i.dataset.label}: ${i.parsed.y.toFixed(1)} km/h`,
            },
          },
        },
        scales: {
...
function renderStrengthAnalytics(sessions) {
  ...
  createChart("muscle", $("#muscle-chart"), {
      ...
      datasets: [
        {
          label: "Max Weight (kg)",
          data: sorted.map(([, w]) => w),
          backgroundColor: "rgba(134,200,163,0.4)",
          borderColor: "#86C8A3",
          borderWidth: 1,
          borderRadius: 6,
        },
    ...
    options: {
      ...
      maintainAspectRatio: false,
      scales: {
        x: { ticks: { callback: (v) => v + " kg" }, beginAtZero: true },
        y: { ticks: { font: { size: 11 } } },
      },
      plugins: { tooltip: { callbacks: { label: (i) => `${i.parsed.x} kg` } } },
    },
  });
}
...
function renderRecentExerciseTabs() {
  ...
  sortedDesc().forEach((s) =>
    s.entries.forEach((e) => {
      if (!latest.has(e.exercise))
        latest.set(e.exercise, { session: s, entry: e });
    }),
  );
  if (!latest.size) {
    out.innerHTML =
      '<p class="muted" style="padding:8px;">No exercise history yet.</p>';
    return;
  }
  out.innerHTML = [...latest.entries()]
    .slice(0, 15)
    .map(([name, data]) => {
      const days = Math.max(
        0,
        Math.floor(
          (new Date(today) - new Date(data.session.sessionDate)) / dayMs,
        ),
      );
      const hue = Math.max(0, 120 - days * 6);
      const detail =
        data.entry.type === "strength"
...
function renderSessions() {
  ...
  if (!sessions.length) {
    list.innerHTML =
      '<li class="muted" style="padding:20px;text-align:center;">No sessions yet. Tap + to add one!</li>';
    return;
  }
  list.innerHTML = sessions
    .map((s) => {
      ...
      const tags = types
        .map((t) => `<span class="session-tag">${emoji[t] || ""} ${t}</span>`)
        .join("");
...
function showSessionDetail(id) {
  ...
  const body = $("#detail-body"),
    vol = strengthVolume(s);
  let html =
    `<div class="detail-meta"><div class="detail-meta-item"><span class="detail-meta-label">Date</span><span class="detail-meta-value">${dateFmt(s.sessionDate)}</span></div>` +
    (s.weightKg != null
      ? `<div class="detail-meta-item"><span class="detail-meta-label">Weight</span><span class="detail-meta-value">${s.weightKg} kg</span></div>`
      : "") +
    `${vol ? `<div class="detail-meta-item"><span class="detail-meta-label">Total Volume</span><span class="detail-meta-value">${vol.toLocaleString()} kg</span></div>` : ""}</div>${s.notes ? `<div style="color:var(--muted);font-size:0.85rem;font-style:italic;">"${s.notes}"</div>` : ""}`;
  for (const e of s.entries) {
    const icon = { cardio: "🏃", strength: "💪", hold: "🧘" }[e.type];
    html += `<div class="detail-entry"><div class="detail-entry-title">${icon} ${e.exercise}</div>`;
    if (e.type === "cardio") {
      html += `<div class="detail-entry-stat">${e.durationMin ? `Duration: ${parseFloat(e.durationMin.toFixed(2))} min<br>` : ""}${e.distanceKm ? `Distance: ${e.distanceKm} km<br>` : ""}Speed: ${calcSpeed(e).toFixed(1)} km/h</div>`;
    } else if (e.type === "strength") {
      html += `<div class="detail-entry-stat">Muscle: ${e.muscle || "N/A"}</div>`;
      if (e.sets?.length) {
        html += `<table class="detail-sets-table"><thead><tr><th>Set</th><th>Weight</th><th>Reps</th><th>Volume</th></tr></thead><tbody>${e.sets.map((set, i) => `<tr><td>${i + 1}</td><td>${set.weightKg} kg</td><td>${set.reps}</td><td>${(set.weightKg * set.reps).toLocaleString()} kg</td></tr>`).join("")}</tbody></table>`;
      }
    } else {
...
function addEntryCard(type, data = null) {
  ...
  const icons = { cardio: "🏃", strength: "💪", hold: "🧘" },
    labels = { cardio: "Cardio", strength: "Strength", hold: "Timed Hold" };
  let f = "";
  if (type === "cardio") {
...
function wireSetRemove(c) {
  if (!c) return;
  c.querySelectorAll(".set-remove").forEach((b) => {
    b.onclick = () => {
      b.closest(".set-row").remove();
      c.querySelectorAll(".set-row").forEach((r, i) => {
...
function wireSmartSearch(card, type) {
  const input = card.querySelector(".exercise-search"),
    sugDiv = card.querySelector(".search-suggestions");
  ...
  input.addEventListener("input", () => {
    ...
    const m = db.filter((i) => i.name.toLowerCase().includes(q)).slice(0, 8);
    if (!m.length) {
      sugDiv.classList.remove("open");
      return;
    }
    sugDiv.innerHTML = m
      .map(
        (i) =>
          `<div class="search-suggestion" data-name="${i.name}" ${i.muscle ? `data-muscle="${i.muscle}"` : ``}>${i.name}${i.muscle ? ` <span style="color:var(--muted);font-size:0.75rem;">(${i.muscle})</span>` : ""}</div>`,
      )
      .join("");
    sugDiv.classList.add("open");
  });
  sugDiv.addEventListener("click", (e) => {
    const i = e.target.closest(".search-suggestion");
    if (!i) return;
...
function resetForm() {
  $("#session-form").reset();
  $("#session-form").sessionDate.value = today;
...
function openForEdit(id) {
  ...
  f.sessionDate.value = s.sessionDate;
  f.weightKg.value = s.weightKg != null ? s.weightKg : "";
...
function collectEntries() {
  ...
  for (const card of cards) {
    ...
    if (type === "cardio") {
      const exercise =
        card.querySelector(".exercise-search")?.value?.trim() || "Cardio";
      const dur = Number(card.querySelector(".entry-dur")?.value) || 0,
        dist = Number(card.querySelector(".entry-dist")?.value) || 0,
        speed = Number(card.querySelector(".entry-speed")?.value) || 0;
      if (!dur && !dist) {
        errors.push("Cardio must include duration or distance.");
        continue;
      }
      entries.push({
        type: "cardio",
        ...
        note: card.querySelector(".entry-note")?.value?.trim() || "",
      });
    } else if (type === "strength") {
      const exercise =
        card.querySelector(".exercise-search")?.value?.trim() || "Strength";
      const muscle = card.querySelector(".entry-muscle")?.value || "Core";
      const sets = [];
      card.querySelectorAll(".set-row").forEach((row) => {
        const w = Number(row.querySelector(".set-weight")?.value) || 0,
          r = Number(row.querySelector(".set-reps")?.value) || 0;
        if (r > 0) sets.push({ weightKg: w, reps: r });
      });
      if (!sets.length) {
        errors.push(
          `Strength entry "${exercise}" needs at least 1 set with reps.`,
        );
        continue;
      }
      entries.push({
      ...
      const exercise =
        card.querySelector(".exercise-search")?.value?.trim() || "Hold";
      entries.push({
        type: "hold",
        exercise,
        sets: Number(card.querySelector(".entry-hold-sets")?.value) || 1,
        seconds: Number(card.querySelector(".entry-hold-secs")?.value) || 0,
        note: card.querySelector(".entry-note")?.value?.trim() || "",
      });
    }
  }
...
$("#session-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await saveSessionToDb({
      sessionDate: $("#session-form").sessionDate.value,
      weightKg: wVal !== "" ? Number(wVal) : null,
      notes: $("#session-form").notes.value || "",
      entries,
    });
    $("#session-dialog").close();
    renderAll();
    showToast(
      state.editingId ? "Session updated!" : "Session saved!",
      "success",
    );
  } catch (err) {
    $("#form-error").textContent =
...
$("#sessions-list").addEventListener("click", (e) => {
  const r = e.target.closest(".session-row");
  if (r) showSessionDetail(r.dataset.sessionId);
});
$("#detail-close").addEventListener("click", () => $("#detail-dialog").close());
$("#detail-delete").addEventListener("click", async () => {
  const id = state._detailSessionId;
  if (!id) return;
  if (!confirm("Delete this session? This cannot be undone.")) return;
  try {
...
$("#plank-plus")?.addEventListener("click", async () => {
  ...
  if (!ts) {
    ts = {
      ...
      weightKg: null,
      notes: "",
      entries: [{ type: "hold", exercise: "Plank", sets: 1, seconds: 60 }],
    };
  } else {
    // deep clone entries so we don't accidentally mutate state before saving
    ts = { ...ts, entries: JSON.parse(JSON.stringify(ts.entries)) };
    const pIdx = ts.entries
      .map((e, i) =>
        e.type === "hold" && e.exercise.toLowerCase() === "plank" ? i : -1,
      )
      .filter((i) => i !== -1)
      .pop();
    if (pIdx !== undefined) {
      ts.entries[pIdx].sets++;
    } else {
      ts.entries.push({
        type: "hold",
        exercise: "Plank",
...
$$(".btn-entry-add").forEach((b) =>
  b.addEventListener("click", () => addEntryCard(b.dataset.entryType)),
);
...
$("#profile-form").addEventListener("submit", (e) => {
  e.preventDefault();
  saveProfile({
    displayName: $("#display-name").value.trim() || "Athlete",
...
_supabase.auth.getSession().then(({ data: { session } }) => {
  if (session?.user)
    hydrateSignedInUser(session.user).catch((err) => {
      console.error("Init hydrate failed:", err);
      renderAll();
    });
  else showAuthDialog();
});
